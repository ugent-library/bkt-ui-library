...
Based on workshop: research output type redesign:
decisions, principles, obligations.
...
This document captures the decisions made on the type system.
Find the design principles that constrain them in WORK-TYPES-RULES.md,
and the external reporting obligations that must be satisfied in WORK-TYPES-OBLIGATIONS.md.
...
**Medium**
A controlled vocabulary field applying to `research-communication`. Captures the
...
- **Letter and note** — not separate types. Both are `journal-article`. Letters and
  ...
  principle 3, and were dropped in v1.3. A subtype-as-field replacement
...
### `journal-letter` and `journal-note` dropped (v1.3)
