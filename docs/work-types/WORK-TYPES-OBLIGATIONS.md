...
This document captures the external reporting obligations that must be satisfied.
Find the design principles that constrain them in WORK-TYPES-RULES.md,
and the decisions made on the type system in WORK-TYPES.md.
...
- Journal articles, including reviews (peer-reviewed, 4+ pages, in approved journal list — letters and notes also fall under `journal-article` since v1.3)
...
as separate types in v1.3 — they are `journal-article` records.
