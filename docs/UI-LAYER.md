...
7. **Public information is never lazy-loaded** (M, 2026-08-06). Crawlable content — works lists, related research, any metadata — must be in the served HTML: scholarly crawlers and harvesters (Google Scholar, OAI-PMH, OpenAIRE) don't reliably execute JS; Googlebot does, but is the exception, not the contract. No `hx-trigger="load"`, `"revealed"` or `"intersect"` on public information. Interaction-only widgets (add-to-list panel contents, cite modal internals) may load on demand.
