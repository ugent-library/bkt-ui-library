...
| `jcr_impact_factor` | Journal impact factor | public advanced if Raven carries it; backoffice | Numeric journal metric used by one observed human query. | No Raven field found in current metadata docs | `jcr_impact_factor`; legacy alias: `jcr.impact_factor` | Needs Raven source, if exposed | decimal number | `is more than`, `is less than`, `is between` | numeric input/range | No value list in phase 1. | `jcr.impact_factor > 5` | Same exposure/source decision as JCR category. |
