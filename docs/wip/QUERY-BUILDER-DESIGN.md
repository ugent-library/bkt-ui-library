...
4. **Test against a record (phase 2).** Pick a known publication; per condition show pass/fail: "matches ✓ / doesn't match ✗ because condition 2 fails". Makes query debugging trivial.
...
6. **Goal-first framing.** Above the rows: "Show publications where …" — anchors what AND means (Zapier's "Only continue if…" lesson).
