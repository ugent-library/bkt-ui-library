...
- **Person tokens** are `badge text-bg-primary-light` + `data-qb-token`, sitting in the value cell beside a `badge badge--outline` button that opens the person picker.
...
- **Rich checklist rows.** A checklist row's label may carry the people-row content: `people-result__name` + `people-result__meta` inside the `form-check-label`, checkbox in place of the user icon. Rows align flex-start so the box sits on the label's first line. Canonical example: `patterns/people-search.html`.
