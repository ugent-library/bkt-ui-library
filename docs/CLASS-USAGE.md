...
**Query builder** — the editor is `card rounded-0` + `card-header bg-transparent p-3`; the blank state's start cards are `card text-start bt-query-builder__start` (the custom class keeps the tighter radius and hover); person tokens are `badge text-bg-primary-light` + `data-qb-token` inside a `form-control bt-query-builder__people-input` wrapper; the role is a caret-less muted `bt-query-builder__role` select (own quiet SVG caret, no dotted rule); the field chooser is a `bt-panel bt-panel--xwide` composed on `dropdown-menu` (see Panel); field-list groups are `min-w-0` + `data-qb-choice-group` holding `ul.list-unstyled.d-grid.gap-1`. The row's actions cell is a ⋯ menu (only "Add an 'or'") plus a dedicated `btn-ghost` remove button carrying `if-delete` — the remove control always names its condition. Pattern page: `patterns/query-builder.html`.

**Panel** — `bt-panel` is the generic popover panel (title + one or more bodies + optional actions footer). Sizes to content by default; `bt-panel--wide` (480px) is for panels whose body is swapped while the user types — the filter editor and the add-to-list picker — because a content-sized panel resizes on every keystroke. `bt-panel--xwide` (40rem) is for panels laying out columns — the query builder's field chooser. Body layouts (`--checklist/--boolean/--year/--form/--grid`) are generic; `--grid` is a three-column group grid (one column below `lg`). **Search-first shape:** stack two bodies — a plain `bt-panel__body` holding the search field (it stays put) above a `bt-panel__body--checklist` or `--grid` (it scrolls); each body draws its own top border, so the divider comes for free — the bodies must be direct children of `bt-panel`, and a first body with no title above it takes `border-top-0`. Used by the add-to-list picker, the `filter-bar.js` filter editors (search-within), and the query builder's chooser (`dropdown-menu p-0 bt-panel bt-panel--xwide` + `role="dialog"` + `aria-label`). Action rows are plain `.dropdown-item`; `_panel.scss` re-declares their padding, since Bootstrap takes it from variables only declared on `.dropdown-menu`. Defined in `patterns/_panel.scss`.
