...
- **Rich checklist rows.** A checklist row's label may carry the people-row content: `people-result__name` + `people-result__meta` inside the `form-check-label`, checkbox in place of the user icon. Rows align flex-start so the box sits on the label's first line. A label that runs to several lines makes the row about three times a plain one, so the 240px body shows roughly three of them and scrolls — that is the settled trade: a taller body puts a scroll inside the dialog's own. Canonical example: `patterns/people-search.html`.
